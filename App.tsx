...
const INITIAL_ELEMENTS: CanvasElement[] = [
  ...
  { id: '2', type: 'note', position: { x: 300, y: 80 }, width: 280, height: 240, rotation: -10, zIndex: 2, content: '🕹️ CONTROL / 控制: \n\n● Pan / 平移:\n   Hold [SPACE] or [Middle Mouse Button]\n   按住 [空白鍵] 或 [滑鼠中鍵]\n\n● Zoom / 縮放: [SCROLL] / [滾輪]\n\n● Options / 選項: [Right-click] / [右鍵]', color: 'bg-green-500' },
...
const translations: Record<string, Record<string, string>> = {
  en: {
    ...
    speedFlash: '⚡ Speed (Flash)',
    qualityPro: '💎 Quality (Nano Banana 2)',
    usingFlash: 'Using Gemini 3 Flash (Fast & Free)',
    usingPro: 'Using Nano Banana 2 (Paid Key Required)',
    ...
    usingDefaultKey: 'Using Default API Key',
  },
  zh: {
    infiniteCanvas: '無限畫布',
    ...
    modelPriority: '生圖模型優先級',
    speedFlash: '⚡ 快捷 (Flash)',
    qualityPro: '💎 畫質 (Nano Banana 2)',
    usingFlash: '使用 Gemini 3 Flash (快速免費)',
    usingPro: '使用 Nano Banana 2 (高畫質，需付費 Key)',
    imageSize: '影像尺寸',
    importTitle: '匯入畫布',
    ...
    fitToScreen: '最適大小 (居中顯示)',
    customApiKey: '自訂 API 金鑰',
    usingCustomKey: '正使用自訂 API 金鑰',
    usingDefaultKey: '使用系統預設 API 金鑰',
  }
};
...
const App: React.FC = () => {
  ...
  const handleAiError = useCallback(async (error: any) => {
    ...
    if (msg.includes('leaked') || msg.includes('403') || error?.status === 403 || error?.code === 403) {
      if ((window as any).aistudio?.openSelectKey) {
        const shouldSelect = window.confirm("您的 API Key 可能已失效或被回報為外洩。是否要重新選擇一個有效的 API Key？\n\n(Your API Key may be invalid or reported as leaked. Would you like to select a new one?)");
        if (shouldSelect) {
    ...
    // Handle other errors with optional translation
    let zhMsg = msg;
    if (language === 'zh') {
      try {
        const ai = getAi();
        if (ai) {
          const translationResponse = await ai.models.generateContent({
            ...
            contents: `Translate this technical AI error message into clear, user-friendly Traditional Chinese (Taiwan). Only return the translated text: "${msg}"`,
            config: {
              systemInstruction: "You are a helpful assistant that translates technical error messages into user-friendly Chinese. Keep it concise.",
            }
          });
          zhMsg = translationResponse.text?.trim() || msg;
        }
      } catch (transError) {
  ...
  const handleFitToScreen = () => canvasApiRef.current?.fitToScreen();
  ...
  const handleGenerate = useCallback(async (selectedElements: CanvasElement[]) => {
    if (generationModel === 'pro' && (window as any).aistudio) {
    ...
    if (!currentAi) {
      if (generationModel === 'pro' && (window as any).aistudio) {
        ...
        alert("API Key is missing. Please set it in the environment or select one.");
      }
      return;
    }
  ...
  }, [getAi, imageStyle, imageAspectRatio, generationModel, imageSize, handleAiError]);
  ...
  return (
    <main className="relative w-screen h-screen bg-gray-100 font-sans" onClick={() => setContextMenu(null)}>
      ...
      >
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-[10px] text-blue-500 uppercase tracking-widest font-black">Ver 3.1 Flash <span className="text-gray-300">| Nano Banana 2</span></p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
            ...
            <p className="text-[10px] text-center mt-1.5 text-gray-500 font-medium">
                {generationModel === 'flash' ? t('usingFlash') : t('usingPro')}
            </p>
        </div>
         ...
         <div className="flex flex-col gap-2 border-t border-gray-100 pt-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="pt-2 border-t border-gray-100">
                <h2 className="text-xs font-bold text-gray-400 mb-2">{t('settings')}</h2>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={toggleLanguage} className="px-3 py-2 text-sm font-bold bg-gray-100 rounded-lg">{t('changeLanguage')}</button>
                  <button onClick={() => setPanelAlignment(p => p === 'left' ? 'right' : 'left')} className="px-3 py-2 text-sm font-bold bg-gray-100 rounded-lg">{panelAlignment === 'left' ? t('alignPanelRight') : t('alignPanelLeft')}</button>
                </div>
                <button
                  onClick={() => setIsApiKeyModalOpen(true)}
                  className="w-full mt-2 px-3 py-2 text-xs font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-200/50 rounded-lg flex items-center justify-center gap-2 transition-all"
                >
                  <span>🗝️</span> {t('customApiKey')}
                  {localStorage.getItem('CUSTOM_GEMINI_API_KEY') ? (
                    <span className="w-2 h-2 rounded-full bg-green-500" title={t('usingCustomKey')} />
                  ) : (
                    <span className="w-2 h-2 rounded-full bg-gray-300" title={t('usingDefaultKey')} />
                  )}
                </button>
              </div>
      ...
      <Minimap elements={elements} viewport={viewport} onPanTo={handlePanTo} onZoomIn={handleZoomIn} onZoomOut={handleZoomOut} onFitToScreen={handleFitToScreen} usageStats={usageStats} t={t} />
        ...
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setGeneratedImages(null)}>
          <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setGeneratedImages(null)} className="p-2 hover:bg-gray-100 rounded-full transition-colors text-gray-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-2 custom-scrollbar">
              {generatedImages.map((imgSrc, index) => (
                <div key={index} className="group relative bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 flex flex-col">
                  ...
                  <div className="p-3 flex gap-3 bg-white border-t">
                    <button onClick={() => addGeneratedImageToCanvas(imgSrc)} className="flex-1 py-2.5 bg-green-500 text-white font-bold rounded-md hover:bg-green-600 transition-all shadow-sm">{t('addToCanvas')}</button>
                    <button onClick={() => downloadGeneratedImage(imgSrc)} className="flex-1 py-2.5 bg-blue-600 text-white font-bold rounded-md hover:bg-blue-700 transition-all shadow-sm">{t('download')}</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {contextMenu && (
        <ContextMenu
          ...
          actions={{
            addNote, addArrow, addDrawing, editDrawing: handleEditDrawing,
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-2xl w-full flex flex-col gap-6 transform animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button 
                ...
                className="flex-1 py-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl transition-all shadow-lg active:scale-[0.98] flex items-center justify-center gap-2"
              >
                <span>🗝️</span> {t('customApiKey')}
              </button>
              <button 
                onClick={() => setAiErrorMessage(null)}
                className="flex-1 py-4 bg-gray-900 text-white font-black rounded-2xl hover:bg-black transition-all shadow-lg active:scale-[0.98]"
              >
                {t('close')}
              </button>
            </div>
